...
public class AbilityDAO
{
	...
	public void put(long player_id)
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("insert into cooldowns values (?, null, null);");
	...
	public Long[] get(long player_id)
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("select * from cooldowns where player_id = ?;");
			ps.setLong(1, player_id);
	...
	public List<Long> expireFind()
	{
		...
		String select_query = "select player_id from cooldowns where find_expiration < ?;";
		String update_query = "update cooldowns set find_expiration = null where find_expiration < ?;";
	...
	public void updateFind(long player_id, long expiration_ts)
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("update cooldowns set find_expiration = ? where player_id = ?;");
	...
	public List<Long> expirePockets()
	{
		List<Long> expires = new ArrayList<>();
		long nowTs = System.currentTimeMillis();
		String update_query = "update cooldowns set pockets_expiration = null where pockets_expiration < ?;";
		try
		{
	...
	public void updatePockets(long player_id, long expiration_ts)
	{
		try
		{
			PreparedStatement ps = connection.prepareStatement("update cooldowns set pockets_expiration = ? where player_id = ?;");
			ps.setString(1, DatabaseDateMediator.ms_to_string(expiration_ts));
			ps.setLong(2, player_id);
			ps.execute();
		}
	...
	private Long read_ts(ResultSet rs, String column_name) throws SQLException
	{
		...
		if (date_UTC_string != null)
		{
			...
			{
				System.err.printf("Error while parsing %s date from database, got:\n%s\n", column_name, date_UTC_string);
			}
			catch (Exception ex)
			{
