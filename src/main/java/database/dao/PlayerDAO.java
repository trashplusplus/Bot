...
public class PlayerDAO implements IPlayerDAO
{
	...
	public void put(Player player)
	{
		...
		try
		{
			ps = connection.prepareStatement("insert into players (id, xp, 'level', needle, name, balance, registered) values (?, ?, ?, ?, ?, ?, ?);");
			...
			ps.setLong(6, player.needle);
			ps.setInt(7, player.getState() == Player.State.awaitingNickname ? 0 : 1);
			ps.execute();
		...
		synchronized (connection)
		{
			try
			{
				...
				String update_players =
						"update players set xp = ?, 'level' = ?, name = ?, balance = ?, needle = ?, registered = ? where id = ?;";
	...
	private Player form(ResultSet rs) throws SQLException
	{
		...
		Player player = new Player(id, xp, level, username, balance, needle, state, inventory, stats, host);
